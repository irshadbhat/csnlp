...
class Parser(object):
    def __init__(self, model=None, meta=None, new_meta=None, test=False):
        ...
        self.xps_pW2 = self.model.add_parameters((self.meta.xn_tags, self.meta.p_hidden))
        ...
        self.XPAD = self.model.add_parameters(self.meta.lstm_wc_dim*2)
        if test and model:
            self.model.populate('%s.dy' %model)
    ...
    def feature_extraction(self, sentence):
        ...
        for xi,node in zip(ps_bi_exps, sentence):
            xh = self.ps_W1 * xi
            #xh = meta.activation(xh) + self.ps_b1
            pos_hidden.append(xh)

        # get word/char embeddings (stacked)
        xcembs = self.get_char_embds(sentence, self.xcf_init, self.xcb_init)
        ...
        for xi,node in zip(xbi_exps, sentence):
            xh = self.xps_W1 * xi
            #xh = meta.activation(xh) + self.xps_b1
    ...
    def predict(self, configuration, pr_bi_exps, xpr_bi_exps):
        ...
        yi = dy.concatenate([xpr_bi_exps[id-1] if id > 0 else self.pad for id, rform in rfeatures])
        xh = self.pr_W1 * xi
        xi = dy.concatenate([yi, xh])
        xh = self.xpr_W1 * xi
        xh = meta.activation(xh) + self.xpr_b1
        return self.xpr_W2*xh + self.xpr_b2 
...
def Train(sentence, epoch, dynamic=True):
    ...
    while not parser.transitionSystem.inFinalState(configuration):
        xo = parser.predict(configuration, pr_bi_exps, xpr_bi_exps)
...
def build_dependency_graph(parser, graph):
    ...
    while not parser.transitionSystem.inFinalState(configuration):
        output_probs = parser.predict(configuration, pr_bi_exps, xpr_bi_exps)
...
def read(fname, palgo):
    ...
    for i,sentence in enumerate(inputGenTrain):
        ...
        if palgo == 'swap':
            ...
            try:
                graph = graph[:1]+projectivize(graph[1:-1])+graph[-1:]
            except:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--elimit', type=int, default=100000, help='load only top-n pretrained word vectors (default=100000)')
